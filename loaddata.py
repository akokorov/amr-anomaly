...
df = pd.DataFrame(columns=['Date','Ship to','Description','Volume(MMSCFD)'])
...
for year_folder in listdir(main_path):
    ...
    for month_folder in listdir(join(year_path)):
        ...
        for file_name in listdir(month_path) :
            ...
            if isfile(file_path):
                df = df.append(pd.read_csv(file_path, sep='|',engine='python')[['Date','Ship to','Description','Volume(MMSCFD)']].iloc[:])
...
df_seq = pd.DataFrame(columns=['Date','Ship to','seq01','seq02','seq03','seq04','seq05','seq06','seq07','seq08','seq09',
                               'seq10','seq11','seq12','seq13','seq14'])
...
for factory_code in require_list:
    ...
    for i in range(len(df_factory)-14):
        df_seq1 = df_factory.iloc[i:i+14]['Volume(MMSCFD)']
        ...
        if False not in np.squeeze(df_seq1 >= 0):
            ...
            df_seq1 = pd.DataFrame(df_seq1,columns=['seq01', 'seq02', 'seq03', 'seq04', 'seq05', 'seq06', 'seq07',
                                                    'seq08','seq09','seq10','seq11','seq12','seq13','seq14'])

            df_seq1['Date'] = df_factory.iloc[i]['Date']
            df_seq1['Ship to'] = factory_code

            df_seq = df_seq.append(df_seq1, sort=True)
    print(factory_code, 'done')

df_seq.to_csv('seq_data_congen14.csv')